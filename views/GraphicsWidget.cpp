...
{
    int TAG_TABLE_WIDTH = 100;
    int ANC_TABLE_WIDTH = 50;
    ui->setupUi(this);
    ...
    tableHeader << "Name" << "X(m)" << "Y(m)" << "Z(m)" << "R95(m)"
                << "Anc 0\n range (m)" << "Anc 1\n range (m)" << "Anc 2\n range (m)" << "Anc 3\n range (m)"
                   ;
    ...
    qDebug()<<"screen pix:"<<mm.width ()<<mm.height ();//主屏幕分辨率的大小

    TAG_TABLE_WIDTH = TAG_TABLE_WIDTH*(mm.width() / 1920.0);
    ANC_TABLE_WIDTH = ANC_TABLE_WIDTH*(mm.width() / 1920.0);

    //ui->tagTable->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);    //x先自适应宽度
    //ui->tagTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::ResizeToContents);     //然后设置要根据内容使用宽度的列


    ui->tagTable->setColumnWidth(ColumnID,TAG_TABLE_WIDTH*0.8);//ID
    ui->tagTable->setColumnWidth(ColumnX,TAG_TABLE_WIDTH*0.7); //x
    ui->tagTable->setColumnWidth(ColumnY,TAG_TABLE_WIDTH*0.7); //y
    ui->tagTable->setColumnWidth(ColumnZ,TAG_TABLE_WIDTH*0.7); //z
    ...
    ui->tagTable->setColumnWidth(ColumnR95,TAG_TABLE_WIDTH*0.8); //R95

    ui->tagTable->setColumnWidth(ColumnRA0,TAG_TABLE_WIDTH);
    ui->tagTable->setColumnWidth(ColumnRA1,TAG_TABLE_WIDTH);
    ui->tagTable->setColumnWidth(ColumnRA2,TAG_TABLE_WIDTH);
    ui->tagTable->setColumnWidth(ColumnRA3,TAG_TABLE_WIDTH);
    ...
    anchorHeader << "Anc ID" << "X\n(m)" << "Y\n(m)" << "Z\n(m)"
                    << "T0\n(cm)" << "T1\n(cm)" << "T2\n(cm)" << "T3\n(cm)"
                    << "T4\n(cm)" << "T5\n(cm)" << "T6\n(cm)" << "T7\n(cm)" ;
    ui->anchorTable->setHorizontalHeaderLabels(anchorHeader);

    ui->anchorTable->setColumnWidth(ColumnID,ANC_TABLE_WIDTH*2);    //ID
    ui->anchorTable->setColumnWidth(ColumnX,ANC_TABLE_WIDTH); //x
    ui->anchorTable->setColumnWidth(ColumnY,ANC_TABLE_WIDTH); //y
    ui->anchorTable->setColumnWidth(ColumnZ,ANC_TABLE_WIDTH); //z
    ui->anchorTable->setColumnWidth(4,ANC_TABLE_WIDTH); //t0
    ui->anchorTable->setColumnWidth(5,ANC_TABLE_WIDTH); //t1
    ui->anchorTable->setColumnWidth(6,ANC_TABLE_WIDTH); //t2
    ui->anchorTable->setColumnWidth(7,ANC_TABLE_WIDTH); //t3
    ui->anchorTable->setColumnWidth(8,ANC_TABLE_WIDTH); //t4
    ui->anchorTable->setColumnWidth(9,ANC_TABLE_WIDTH); //t5
    ui->anchorTable->setColumnWidth(10,ANC_TABLE_WIDTH); //t6
    ui->anchorTable->setColumnWidth(11,ANC_TABLE_WIDTH); //t7
...
void GraphicsWidget::addNewTag(quint64 tagId)
{
    ...
    Tag *tag;
    int tid = tagId & 0xf;
    QString taglabel = QString("Tag %1").arg(tagId); //taglabels.value(tagId, NULL);//LLP
    ...
    else
    {
        ...
        if (tag)
        {
            ...
            //update Tag/Anchor range value in the table
            {
                ...
                if(ridx != -1)
                {
                    ui->tagTable->item(ridx,ColumnRA0+anc)->setText(QString::number(range, 'f', 2));
                }
