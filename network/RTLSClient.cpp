...
kal_t kal;
int   start_rec=0;
QString Name_Of_ComPort;
void RTLSClient::newData()
{
    QByteArray data; //静态变量！！在串口只发送一半的时候用来累加数据
    //qDebug()<< "Name_Of_ComPort" << Name_Of_ComPort;
    if( Name_Of_ComPort == DEVICE_STR2)
    {
        if(_serial->bytesAvailable()>=65)
        {
    ...
    qDebug()<< "11111111111111111"<<data << 222 << length;
    int offset = 0;
    QString statusMsg;
#if (DEBUG_FILE==1)
    if(_fileDbg)
        ...
        if(type == 'c') //if 'c' these reports relate to tag <-> anchor ranges
        {
            ...
            for(int k=0; k<MAX_NUM_ANCS; k++)
            {
                if((0x1 << k) & mask) //we have a valid range
                {
                        ...
                        kal.x_mid[tid][k]  = kal.x_last[tid][k];
                        kal.p_mid[tid][k]  = kal.p_last[tid][k] + kal.Q;
                        kal.kg[tid][k]     = kal.p_mid[tid][k] / (kal.p_mid[tid][k] + kal.R);
                        //z_measure = z_real + frand()*0.3;//测量值
                        kal.x_now[tid][k]  = kal.x_mid[tid][k] + kal.kg[tid][k]*(range[k] - kal.x_mid[tid][k]);//估计出的最有值
                        kal.p_now[tid][k]  = (1 - kal.kg[tid][k])*kal.p_mid[tid][k];//最优值对应的协方差
                        kal.p_last[tid][k] = kal.p_now[tid][k];
                        kal.x_last[tid][k] = kal.x_now[tid][k];
                        range[k] = kal.x_now[tid][k];
